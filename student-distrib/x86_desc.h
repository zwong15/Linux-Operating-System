...
typedef struct page_directory_t {
  uint8_t present  : 1;  // 1 if we are using, 0 if we are not
  uint8_t R_W      : 1;  // always set to 1
  uint8_t U_S      : 1;  // 0 for kernal, 1 for else
  uint8_t PWT      : 1;  // always set to 0
  uint8_t PCD      : 1;  // 0 for video memory and 1 for else. EDIT: THIS IS BACKWARDS
  uint8_t accessed : 1;  // we will not use this bit
  uint8_t dirty    : 1;  // always set to 0
  uint8_t size     : 1;  // high if using 4MB pages and low for 4kB pages
  uint8_t global   : 1;  // will only be set high for kernal
  uint8_t avail3    : 1;  // we will not use these bits
  uint8_t avail2    : 1;  // we will not use these bits
  uint8_t avail1    : 1;  // we will not use these bits
      uint32_t pointer : 20; // pointer to page table if size = 0, else top 10 bits
                             // for page address and bottom 10 must be set to zero
}page_directory_t;
...
typedef struct page_table_t {
  ...
  uint8_t accessed : 1;  // we will not use this bit
  uint8_t dirty    : 1;  // always set to 0
  uint8_t PAT      : 1;  // always set to 0
  uint8_t global   : 1;  // will only be set high for kernal
  uint8_t avail3    : 1;  // we will not use these bits
  uint8_t avail2    : 1;  // we will not use these bits
  uint8_t avail1    : 1;  // we will not use these bits
      uint32_t offset : 20; // offset to page
}page_table_t;

/* The Page Directory */
page_directory_t page_dir[1024] __attribute__((aligned (4096)));

/* The Page Table for first page directory */
